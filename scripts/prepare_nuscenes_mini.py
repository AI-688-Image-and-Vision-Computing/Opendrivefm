"""
Dataset Curation Pipeline — nuScenes Mini Manifest Builder
===========================================================
Curates the nuScenes v1.0-mini dataset into a structured manifest for
multi-view BEV perception training and evaluation.

Pipeline:
  1. Scene-level train/val splits (prevents data leakage across temporal sequences)
  2. Multi-view camera geometry extraction per sample:
       - Camera intrinsics K_v  (3×3 projection matrix per camera)
       - Camera extrinsics T_ego_cam_v  (4×4 cam→ego rigid transform)
  3. Road geometry alignment: all 6 camera frustums registered to ego-vehicle
     coordinate frame — the foundation for consistent BEV road layout encoding
  4. Manifest output: JSONL rows with image paths, calibration, scene metadata

Each JSONL row includes:
  cams, intrinsics (K_v per camera), extrinsics (T_ego_cam_v per camera)

Keywords: dataset curation, multi-view geometry, road geometry, scene structure,
          camera calibration, ego-vehicle coordinate frame, BEV road layout
"""
